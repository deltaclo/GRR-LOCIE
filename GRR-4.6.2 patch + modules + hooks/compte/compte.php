...
// Hook pour modules externes : liens supplementaires dans "Gerer mon compte"
$d['hookCompteMenu'] = "";
$boutonsPersoAccountMenuManaged = false;
$boutonsPersoLibDir = dirname(__DIR__).'/personnalisation/modules/boutons_perso/lib';
if (
	is_file($boutonsPersoLibDir.'/Config.php')
	&& is_file($boutonsPersoLibDir.'/Repository.php')
	&& is_file($boutonsPersoLibDir.'/ModuleRegistry.php')
	&& is_file($boutonsPersoLibDir.'/Renderer.php')
) {
	require_once $boutonsPersoLibDir.'/Config.php';
	require_once $boutonsPersoLibDir.'/Repository.php';
	require_once $boutonsPersoLibDir.'/ModuleRegistry.php';
	require_once $boutonsPersoLibDir.'/Renderer.php';

	$boutonsPersoModuleActive = grr_sql_query1(
		"SELECT COUNT(*) FROM ".TABLE_PREFIX."_modulesext WHERE nom = ? AND actif = 1",
		"s",
		array(BoutonsPersoConfig::MODULE)
	);
	if (
		(int) $boutonsPersoModuleActive > 0
		&& BoutonsPersoConfig::isEnabled()
		&& BoutonsPersoConfig::accountMenuEnabled()
	) {
		$d['hookCompteMenu'] = BoutonsPersoRenderer::managedAccountMenu();
		$boutonsPersoAccountMenuManaged = true;
	}
}
if (!$boutonsPersoAccountMenuManaged) {
	$resulHook = Hook::Appel("hookCompteMenu");
	$d['hookCompteMenu'] = isset($resulHook['hookCompteMenu']) ? $resulHook['hookCompteMenu'] : "";
}
...
if(in_array($page.".php",$listeFichiers))
	...
	// Hook pour modules externes : pages supplementaires dans "Gerer mon compte"
	$resulHook = Hook::Appel("hookComptePage");
	if (isset($resulHook['hookComptePage']) && $resulHook['hookComptePage'] != "") {
		$d['hookComptePage'] = $resulHook['hookComptePage'];
		echo $twig->render('layout.twig', array('liensMenu' => $menuAdminT, 'liensMenuN2' => $menuAdminTN2, 'd' => $d, 'trad' => $trad, 'settings' => $AllSettings));
	} else
		echo "erreur";
}
?>
