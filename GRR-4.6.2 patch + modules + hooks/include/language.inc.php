...
if (isset($_SESSION['default_language']))
{
	...
	$locale = SecuChaine::Alphanumeric($_SESSION['default_language']);
}
...
// Si l'id du domaine est passé en paramètre, on le récupère
	$subst_id_area = intval(SecuChaine::CleanInput($_GET['area']));
}
else if (isset($_GET['room']))
{
// sinon, on essaye avec l'id de la ressource
	$subst_id_area = mrbsGetRoomArea(intval(SecuChaine::CleanInput($_GET['room'])));
}
 ...
 * @param string $string
 */
function grr_latin1_to_utf8($string)
{
...
function utf8_convert($string)
{
	...
	if (!test_utf8($string)){
		return grr_latin1_to_utf8($string);
	} else{
		return $string;
	}
}
