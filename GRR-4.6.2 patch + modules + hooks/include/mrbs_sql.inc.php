...
function mrbsEntryGetOverloadDesc($id_entry)
{
	...
	if ( $room_id >0 )
	{
		...
		foreach ($fieldslist as $field=>$fieldtype)
		{
			...
			$overload_array[$field]["mail_spec"] = grr_sql_query1("SELECT mail_spec FROM ".TABLE_PREFIX."_overload WHERE id = '".$fieldslist[$field]["id"]."'");
	...
	foreach ($overload_fields_list as $field=>$fieldtype)
	{
		...
		if($overload_data != '')
			if (array_key_exists($id_field,$overload_data))
			{
				...
				$end_string = "@/".$id_field."@";
				// ELM - Gestion des champ additionnels multivalués (438 - 441)
	...
	foreach ($overload_fields_list as $field=>$fieldtype)
	{
		...
		if (array_key_exists($id_field,$overload_data))
		{
			...
			//$overload_data_string .= $begin_string.base64_encode($overload_data[$id_field]).$end_string;
				// ELM - Gestion des champ additionnels multivalués (496 - 500)
				if (is_array($overload_data[$id_field])) {
					$valeurs = implode("|", $overload_data[$id_field]);
					$overload_data[$id_field] = $valeurs;
				}
...
function moderate_entry_do($_id,$_moderate,$_description,$send_mail="yes")
{
	...
	{
		$GLOBALS['grr_moderate_entry_context'] = array(
			'entry_id' => (int) $_id,
			'entry_ids' => $series == 0 ? array((int) $_id) : array_map('intval', $tab_id_moderes),
			'moderate' => (int) $_moderate,
			'decision' => ((int) $_moderate === 1) ? 'accepted' : 'refused',
			'description' => (string) $_description,
			'moderator' => getUserName(),
			'send_mail' => $send_mail
		);
		Hook::Appel("hookModerateEntry");
	}
	//moderation de la ressource
	if ($_moderate != 1)
	{
