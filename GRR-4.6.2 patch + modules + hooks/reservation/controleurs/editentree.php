...
$room_from_request = SecuChaine::GetFormVar('room', 'int');
if ((!isset($room) || empty($room) || $room == -1) && !empty($room_from_request) && $room_from_request != -1) {
  $room = $room_from_request;
}

if ((!isset($room) || empty($room) || $room == -1) && isset($area) && !empty($area) && $area != -1) {
  $room = grr_sql_query1("SELECT min(id) FROM ".TABLE_PREFIX."_room WHERE area_id='".$area."' ORDER BY order_display,room_name");
}

if ((!isset($room_id) || empty($room_id) || $room_id == -1) && isset($room) && !empty($room) && $room != -1) {
  $room_id = $room;
}

if ((!isset($area) || empty($area) || $area == -1) && isset($room) && !empty($room) && $room != -1) {
  $area = mrbsGetRoomArea($room);
}

if (!isset($room) || empty($room) || $room == -1 || !isset($area) || empty($area) || $area == -1) {
  $d['messageErreur'] = get_vocab('ressource_ou_domaine_non_defini');
...
$hour = isset($hour) ? (int) $hour : (int) date('H');
$minute = isset($minute) ? (int) $minute : 0;
$start_hour = $hour;
$start_min = $minute;
...
if (!isset($room_id) || empty($room_id) || $room_id == -1) {
  $room_from_request = SecuChaine::GetFormVar('room', 'int');
  if (isset($room) && !empty($room) && $room != -1) {
    $room_id = $room;
  } elseif (!empty($room_from_request) && $room_from_request != -1) {
    $room_id = $room_from_request;
  } else {
    $room_id = -1;
  }
}

if ((!isset($area) || empty($area) || $area == -1) && $room_id != -1) {
...
// Hook pour modules externes : champs supplementaires dans le formulaire de reservation
$GLOBALS['grr_editentree_context'] = array(
  'room_id' => isset($room_id) ? (int) $room_id : 0,
  'entry_id' => isset($id) ? (int) $id : 0,
  'create_by' => isset($create_by) ? $create_by : '',
  'edit_type' => isset($edit_type) ? $edit_type : '',
  'copier' => isset($copier) ? $copier : ''
);
$GLOBALS['suivi_demandes_editentree_context'] = $GLOBALS['grr_editentree_context'];
$resulHook = Hook::Appel("hookEditEntreeForm");
$d['hookEditEntreeForm'] = isset($resulHook['hookEditEntreeForm']) ? $resulHook['hookEditEntreeForm'] : "";
