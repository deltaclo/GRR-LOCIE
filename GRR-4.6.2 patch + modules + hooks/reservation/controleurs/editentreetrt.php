...
$form_vars = array(
  ...
  'end_hour'           => 'int',
  'end_minute'         => 'int',
  'suivi_demandes_reservation_mode' => 'string',
  'suivi_demandes_existing_id' => 'int',
  'suivi_demandes_title' => 'string',
  'suivi_demandes_description' => 'string',
  'suivi_demandes_priority' => 'string',
  'suivi_demandes_category' => 'string',
  'stagiaire_nom' => 'string',
  'stagiaire_prenom' => 'string',
  'stagiaire_email' => 'string',
  'stagiaire_encadrant' => 'string'
);
...
try {
  ...
  //  echo $page;
   ...
   // echo '1er tests passés';
    ...
    $GLOBALS['grr_editentreetrt_validate_context'] = array(
        'entry_id' => isset($id) ? (int) $id : 0,
        'rooms' => $rooms,
        'create_by' => isset($create_by) ? $create_by : '',
        'name' => isset($name) ? $name : '',
        'description' => isset($description) ? $description : '',
        'start_time' => isset($start_time) ? (int) $start_time : 0,
        'end_time' => isset($end_time) ? (int) $end_time : 0,
        'rep_type' => isset($rep_type) ? (int) $rep_type : 0,
        'beneficiaire' => isset($beneficiaire) ? $beneficiaire : ''
    );
    $resulHook = Hook::Appel("hookEditEntreeValidate");
    $hookErrors = array();
    if (isset($resulHook['hookEditEntreeValidate'])) {
        if (is_array($resulHook['hookEditEntreeValidate'])) {
            $hookErrors = $resulHook['hookEditEntreeValidate'];
        } elseif (trim((string) $resulHook['hookEditEntreeValidate']) !== '') {
            $hookErrors[] = (string) $resulHook['hookEditEntreeValidate'];
        }
    }
    if (count($hookErrors) > 0) {
        $d['err_type'] = 'invalid_parameters';
        $messages = array();
        foreach ($hookErrors as $hookError) {
            $messages[] = htmlspecialchars((string) $hookError, ENT_QUOTES, 'UTF-8');
        }
        $d['err_msg'] = implode('<br>', $messages);
        throw new Exception('erreur');
    }
    $compt_room = 0;
	...
	foreach ($rooms as $room_id)
	{
		...
		if ($rep_type != 0) // Réservation périodique
		{
            ...
            if (isset($id_first_resa) && (int) $id_first_resa > 0) {
                $repeat_id_for_hook = grr_sql_query1(
                    "SELECT repeat_id FROM ".TABLE_PREFIX."_entry WHERE id = ?",
                    "i",
                    array((int) $id_first_resa)
                );
                if ((int) $repeat_id_for_hook > 0) {
                    $res_hook_entries = grr_sql_query(
                        "SELECT id FROM ".TABLE_PREFIX."_entry WHERE repeat_id = ? AND room_id = ? AND supprimer = 0 ORDER BY start_time, id",
                        "ii",
                        array((int) $repeat_id_for_hook, (int) $room_id)
                    );
                    if ($res_hook_entries) {
                        for ($i_hook = 0; ($row_hook = grr_sql_row_keyed($res_hook_entries, $i_hook)); $i_hook++) {
                            $entry_ids_for_hook[] = (int) $row_hook['id'];
                        }
                    }
                }
                if (count($entry_ids_for_hook) === 0) {
                    $entry_ids_for_hook[] = (int) $id_first_resa;
                }
            }
		...
		else // Réservation unique
		{
            ...
            if ($rep_type == 0) {
                ...
                if (isset($id) && (int) $id > 0) {
                    $entry_ids_for_hook[] = (int) $id;
                }
            }
            $id_for_import = ($rep_type == 0) ? $id : 0;
            if (isset($entry_ids_for_hook) && count($entry_ids_for_hook) > 0) {
                $entry_ids_for_hook = array_values(array_unique(array_map('intval', $entry_ids_for_hook)));
                $GLOBALS['grr_editentreetrt_context'] = array(
                    'entry_id' => (int) $entry_ids_for_hook[0],
                    'entry_ids' => $entry_ids_for_hook,
                    'room_id' => (int) $room_id,
                    'create_by' => isset($create_by) ? $create_by : '',
                    'name' => isset($name) ? $name : '',
                    'description' => isset($description) ? $description : '',
                    'start_time' => isset($start_time) ? (int) $start_time : 0,
                    'end_time' => isset($end_time) ? (int) $end_time : 0,
                    'moderate' => isset($entry_moderate) ? (int) $entry_moderate : 0,
                    'send_mail_moderate' => isset($send_mail_moderate) ? (int) $send_mail_moderate : 0,
                    'rep_type' => isset($rep_type) ? (int) $rep_type : 0,
                    'is_repeating' => isset($rep_type) && (int) $rep_type !== 0 ? 1 : 0
                );
                $GLOBALS['suivi_demandes_editentreetrt_context'] = $GLOBALS['grr_editentreetrt_context'];
                Hook::Appel("hookEditEntreeTrt");
            }
