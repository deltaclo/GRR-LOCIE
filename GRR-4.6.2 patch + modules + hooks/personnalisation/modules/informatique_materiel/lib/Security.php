...
class InformatiqueMaterielSecurity
{
    const CSRF_SESSION_KEY = 'informatique_materiel_csrf';
    ...
    public static function role($login = null)
    {
        ...
        return InformatiqueMaterielRepository::roleForLogin($login);
    }
    ...
    public static function canViewUserEquipment($login = null)
    {
        $login = $login === null ? self::currentLogin() : trim((string) $login);
        return $login !== '';
    }
    ...
    {
        if (!isset($_SESSION[self::CSRF_SESSION_KEY]) || preg_match('/^[a-f0-9]{64}$/', (string) $_SESSION[self::CSRF_SESSION_KEY]) !== 1) {
            try {
    ...
    {
        return '<input type="hidden" name="imat_csrf" value="'.htmlspecialchars(self::token(), ENT_QUOTES, 'UTF-8').'">';
    }

    public static function validatePost()
    {
        $expected = isset($_SESSION[self::CSRF_SESSION_KEY]) ? (string) $_SESSION[self::CSRF_SESSION_KEY] : '';
        $received = isset($_POST['imat_csrf']) ? (string) $_POST['imat_csrf'] : '';
